# VPC Network: Main VPC for gcp-project1-442623
...
resource "google_compute_firewall" "allow_postgres_vpc" {
  ...
  source_ranges = ["10.20.0.0/16", "10.10.0.0/16", "100.64.0.0/10"]
  priority      = 900
  direction     = "INGRESS"
}
...
resource "google_compute_firewall" "allow_sql_proxy_egress" {
  ...
  direction          = "EGRESS"
  destination_ranges = ["10.20.0.0/16", "10.10.0.0/16", "100.64.0.0/10"]
  priority           = 1000
}

/* # VPC Peering to terraform-workspace-437404
resource "google_compute_network_peering" "peering_to_project1" {
  name                 = "peering-to-project1"
  network              = google_compute_network.nw1-vpc.id
  peer_network         = "projects/terraform-workspace-437404/global/networks/nw1-vpc"
